...
public class ChatDatabaseHelper {
    ...
    // RIMOSSE LE MAPPE LOCALI: userAvatarCache e userVerifiedCache
    // Ora ci affidiamo interamente alla classe globale UserCache
        ...
        query.orderBy("timestamp", Query.Direction.ASCENDING)
                .addSnapshotListener((value, error) -> {
                    ...
                    for (QueryDocumentSnapshot doc : value) {
                        ...
                        // UTILIZZO DELLA USER CACHE GLOBALE
                        ...
                        // Se l'utente non è in cache, lo scarichiamo
                        if (sender != null && UserCache.getUser(sender) == null) {

                            // Inseriamo un dato temporaneo per fare da "lucchetto" ed evitare di lanciare
                            // query multiple a Firestore per lo stesso utente mentre la prima è in corso.
                            UserCache.putUser(sender, sender, null, false);

                            db.collection("users").document(sender).get().addOnSuccessListener(userDoc -> {
                                if (userDoc.exists() && fragment.isAdded()) {
                                    String uri = userDoc.contains("userPfUri") ? userDoc.getString("userPfUri") : null;
                                    Boolean verifiedObj = userDoc.getBoolean("isVerified");
                                    boolean verified = verifiedObj != null && verifiedObj;
                                    String username = userDoc.contains("username") ? userDoc.getString("username") : sender;

                                    // AGGIORNIAMO LA CACHE GLOBALE CON I DATI REALI
                                    UserCache.putUser(sender, username, uri, verified);

                                    // Aggiorniamo i messaggi già caricati nella lista
